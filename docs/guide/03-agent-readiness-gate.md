...
if $COMPOSE exec -T postgres pg_isready -U threadsflow >/dev/null 2>&1; then
  ...
  if [[ "$l4_token_ok" != "t" ]]; then
    fail "L4 is slated for activation but settings.l4_reply.threads_token is missing. Run: ./scripts/set_secrets.sh --threads-token TOKEN --threads-user-id ID — this writes both threads_creds and l4_reply.threads_token."
  fi

  # Check for Shopee app secret (now written by set_secrets.sh)
  shopee_secret_ok="$($COMPOSE exec -T postgres psql -U threadsflow -d threadsflow -Atc \
    "SELECT COALESCE((value->>'secret') <> '', false) FROM settings WHERE key='shopee_app_secret';" 2>/dev/null || true)"
  if [[ "$REQUIRE_SHOPEE" == "true" && "$shopee_secret_ok" != "t" ]]; then
    warn "Shopee Open API secret not in settings. Run: ./scripts/set_secrets.sh --shopee-app-id ID --shopee-secret SECRET — this now writes both shopee_app_id and shopee_app_secret rows."
  fi
else
