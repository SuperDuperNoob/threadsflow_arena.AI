...
System Settings Control Board" landed on `main`). **Re-verified 2026-08-01 (second pass) against
commits `cbc05da` "fix(wf3_publish): add VIDEO/MIXED_CAROUSEL media type support",
`b315e49` "fix(wf7_l4_reply): add Threads comment ingestion from API", `f448cda`
"fix(product): add WebP/MP4/MOV support and increase file limit to 20", `e4e7b53`
"fix(scripts): write shopee_app_secret row in set_secrets.sh", `2bef5af`
"fix(kb): pass media_kind to describeImage() to skip non-image vision calls", `1d796b4`
"fix(scripts): correct stale l4_config log message in set_secrets.sh", `460c5df`
"fix(config): add missing env vars to .env.example", `b8be4f9`
"fix(settings): remove 'scoring' from SYSTEM_SETTINGS_ALLOWLIST to match UI tabs" on `main`).**
The re-verification pass re-read the live code for every claim these commits could have changed and
updated the affected doc rows below plus the corresponding source files (`README.md`,
`docs/00-START-HERE.md`, `docs/01-architecture.md`, `docs/02-n8n-workflows.md`,
`docs/03-setup-runbook.md`, `docs/07-l4-reply-loop.md`, `docs/08-72h-canary.md`,
`docs/09-wf6-l4-improvements.md`, `docs/11-quick-start.md`, `docs/12-text-variation.md`,
...
|| Media intake | Server accepts image/jpeg, image/png, image/webp, video/mp4, video/quicktime with a 20-file limit and inserts `media_kind` (`services/kb/server.js:400-409`, `436-489`). Browser `/product.html` now accepts JPG/PNG/WebP/MP4/MOV with a 20-file limit (`services/kb/public/product.html:88-112`). |
...
|| Publishing | `wf3_publish.json` now has route branches for `TEXT`, `IMAGE`, `CAROUSEL`, `VIDEO`, and `MIXED_CAROUSEL` with async status polling for video types before `threads_publish` (nodes: `Route by media type`, `Create TEXT container`, `Create IMAGE container`, `Create carousel items`, `Create CAROUSEL container`, `Create VIDEO container`, `Create mixed carousel items`, `Create MIXED_CAROUSEL container`, `Poll video status` with loop). |
...
|| `docs/02-n8n-workflows.md` | **Updated (re-verified)** | Original pass corrected workflow inventory and added explicit VIDEO/MIXED gap. Re-verification pass confirmed video/mixed publishing is now wired in `wf3_publish.json` with `VIDEO`/`MIXED_CAROUSEL` branches and async status polling. Traceable to `n8n/workflows/wf3_publish.json`, `n8n/code/bandit.js:180-198`. |
...
|| `docs/07-l4-reply-loop.md` | **Updated (re-verified)** | Original pass documented the comment-ingestion gap and token mismatch (ingestion gap re-confirmed still open). Re-verification pass marked the token mismatch resolved and confirmed L4 comment ingestion is now implemented in `wf7_l4_reply.json` with nodes for fetching, processing, and upserting Threads comments. Traceable to `scripts/set_secrets.sh:135-141`; ingestion gap resolved by new nodes in `n8n/workflows/wf7_l4_reply.json`. |
...
|| `docs/09-wf6-l4-improvements.md` | **Updated (re-verified)** | Original pass rewrote stale implementation summary and documented token mismatch. Re-verification pass confirmed token mismatch resolved and updated L4 comment ingestion status. Traceable to migration 011, wf6/wf7 JSON, `scripts/bootstrap_n8n.sh`, `scripts/set_secrets.sh`. |
...
|| `docs/11-quick-start.md` | **Updated (re-verified)** | Original pass corrected migration count and added media/schema/browser/video publishing/settings UI status rows. Re-verification pass updated video/mixed publishing status (now wired), browser upload now supports video, L4 ingestion implemented, and Shopee secret key fixed. Traceable to migrations, workflow JSONs, and bootstrap script. |
...
|| `docs/14-agent-autonomous-deploy.md` | **Updated (re-verified)** | Original pass reframed zero-credential autonomy and added video/mixed publisher caveat. Re-verification pass updated video/mixed publishing status (now wired), browser upload now supports video, L4 ingestion implemented, and Shopee secret key fixed. Traceable to env/compose/scripts and workflow audit. |
...
|| `docs/guide/01-credential-sourcing.md` | **Updated (re-verified)** | Credential table verified against current `infra/.env.example`, `scripts/set_secrets.sh`, `infra/docker-compose.yml`, and `services/kb/lib/shopee.js`. Shopee secret key row added to `set_secrets.sh`. Traceable to `scripts/set_secrets.sh:150-161`. |
|| `docs/guide/02-preflight-checklist.md` | **Updated (re-verified)** | Checklist updated for video/mixed publishing now wired, browser video upload supported, L4 ingestion implemented. Traceable to workflow and code changes. |
|| `docs/guide/03-agent-readiness-gate.md` | **Updated (re-verified)** | Gate script updated to check for `shopee_app_secret` in settings, L4 ingestion readiness, and video/mixed publish capability. Traceable to workflow and code changes. |
