...
| Settings UI | `/settings.html` is an LLM settings page only; server routes are `GET /api/config/llm`, `PUT /api/config/llm`, and `/api/config/llm/test` (`services/kb/public/settings.html`; `services/kb/server.js:165-277`). No `PUT /api/config/system/:key` route exists. |
